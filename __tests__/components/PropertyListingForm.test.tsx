...
describe('PropertyListingForm', () => {
  ...
  it('validates numeric fields', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/bedrooms must be a valid number/i)).toBeInTheDocument();
      expect(screen.getByText(/bathrooms must be a valid number/i)).toBeInTheDocument();
      expect(screen.getByText(/size must be a valid number/i)).toBeInTheDocument();
    });
  });
  ...
  it('validates year built range', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/year built must be between 1900 and current year/i)).toBeInTheDocument();
    });
  });
  ...
  it('validates postal code format', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/postal code must be a valid format/i)).toBeInTheDocument();
    });
  });
  ...
  it('handles form submission with valid data', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/title/i), { target: { value: 'Beautiful House' } });
    fireEvent.change(screen.getByLabelText(/description/i), { target: { value: 'A beautiful house in a great location' } });
    fireEvent.change(screen.getByLabelText(/price/i), { target: { value: '500000' } });
    fireEvent.change(screen.getByLabelText(/location/i), { target: { value: 'Amsterdam' } });
    fireEvent.change(screen.getByLabelText(/type/i), { target: { value: 'house' } });
    ...
    fireEvent.change(screen.getByLabelText(/size/i), { target: { value: '150' } });
    fireEvent.change(screen.getByLabelText(/address/i), { target: { value: '123 Main St' } });
    fireEvent.change(screen.getByLabelText(/city/i), { target: { value: 'Amsterdam' } });
    fireEvent.change(screen.getByLabelText(/postal code/i), { target: { value: '1234 AB' } });
    fireEvent.change(screen.getByLabelText(/country/i), { target: { value: 'Netherlands' } });
  ...
  it('shows loading state during submission', async () => {
    ...
    // Fill in required fields
    fireEvent.change(screen.getByLabelText(/title/i), { target: { value: 'Test House' } });
    fireEvent.change(screen.getByLabelText(/description/i), { target: { value: 'A test house description' } });
